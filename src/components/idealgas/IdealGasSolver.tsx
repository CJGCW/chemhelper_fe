...
import { lowestSigFigs, countSigFigs, formatSigFigs } from '../../utils/sigfigs'
...
export default function IdealGasSolver() {
  ...
  function reset() { setResult(null); setResultVar(null); setSteps([]); setError(null); setVerified(null) }
  ...
  const verifiedColors: Record<NonNullable<VerifyState>, string> = {
    correct: '#4ade80',
    incorrect: '#f87171',
    sig_fig_warning: '#fb923c',
  }
      ...
      <div className="flex gap-2">
        ...
        {hasAny && (
          <button onClick={handleClear}
            className="px-4 py-2.5 rounded-sm font-sans text-sm border border-border text-secondary hover:text-primary transition-colors">
            Clear
          </button>
        )}
      </div>
                ...
                {verified && (
                  <span className="font-mono text-xs mb-1" style={{ color: verifiedColors[verified] }}>
                    {verified === 'correct' ? '✓ Correct'
                      : verified === 'sig_fig_warning' ? '⚠ Check sig figs'
                      : '✗ Incorrect'}
                  </span>
                )}
                <span className="font-sans text-xs text-secondary">{varLabel[resultVar]}</span>
                <span className="font-mono text-2xl font-bold" style={{
                  color: verified ? verifiedColors[verified] : 'var(--c-halogen)'
                }}>
                  {result} {resultUnit}
                </span>
              </div>
