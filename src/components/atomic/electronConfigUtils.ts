...
// Common oxidation states by Z — only non-zero charges; 0 (neutral) always available
export const VALID_CHARGES: Partial<Record<number, number[]>> = {
  1:  [-1, 1],           // H
  3:  [1],               // Li
  4:  [2],               // Be
  5:  [3],               // B
...
export function availableCharges(z: number): number[] {
...
function removeElectronsFromConfig(config: SubshellFill[], count: number): SubshellFill[] {
  // Removes from highest-n subshell first (4s before 3d for transition metals)
  const sorted = [...config].sort((a, b) => b.n !== a.n ? b.n - a.n : b.l - a.l)
  const resultMap = new Map(config.map(s => [s.label, { ...s }]))
  let rem = count
  for (const sub of sorted) {
    if (rem <= 0) break
    const s = resultMap.get(sub.label)!
    const take = Math.min(rem, s.electrons)
    s.electrons -= take
    rem -= take
  }
  return [...resultMap.values()].filter(s => s.electrons > 0)
}

// Returns the electron configuration for a species (Z, charge).
// For cations: starts from the neutral (exception-aware) config and removes outermost electrons.
// For anions:  fills in Aufbau order (no exceptions apply to common anion electron counts).
export function configForSpecies(z: number, charge: number): SubshellFill[] {
  if (charge === 0) return computeConfig(z)
  if (charge > 0) return removeElectronsFromConfig(computeConfig(z), charge)
  return computeConfig(z - charge) // z - negative charge = z + |charge|
}

export function unpairedForSpecies(z: number, charge: number): number {
  return configForSpecies(z, charge).reduce((sum, sub) => {
