...
export default function IdentifyReagentPractice({ category = 'all' }: Props) {
  const [q, setQ]             = useState<MechQuestion>(() => generateQuestion('identify-reagent', category)!)
  ...
  function next() {
    setQ(generateQuestion('identify-reagent', category)!)
    setSelected(null)
    setChecked(false)
    setShowSteps(false)
  }
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl">
      <p className="font-sans text-sm text-secondary leading-relaxed">
        Given a reactant and product, identify the reagent or conditions needed.
      </p>
        ...
        >
          ...
          {checked && (
            <div className="flex flex-col gap-1.5">
              ...
              <button onClick={() => setShowSteps(o => !o)} className="self-start font-sans text-xs text-dim hover:text-secondary transition-colors mt-1">
                {showSteps ? '▲ Hide steps' : '▼ Show full solution'}
              </button>
            </div>
          )}
        </motion.div>
        ...
        {showSteps && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.15 }} style={{ overflow: 'hidden' }}>
            <div className="rounded-sm border border-border p-4 flex flex-col gap-2" style={{ background: 'rgb(var(--color-base))' }}>
              ...
              {q.steps.map((step, i) => <p key={i} className="font-sans text-sm text-secondary leading-relaxed">{step}</p>)}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
