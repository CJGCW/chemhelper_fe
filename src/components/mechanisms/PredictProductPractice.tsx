...
export default function PredictProductPractice({ category = 'all' }: Props) {
  const [q, setQ]             = useState<MechQuestion>(() => generateQuestion('predict-product', category)!)
  ...
  function next() {
    setQ(generateQuestion('predict-product', category)!)
    setSelected(null)
    setChecked(false)
    setShowSteps(false)
  }
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl">
      <p className="font-sans text-sm text-secondary leading-relaxed">
        Given the reactants and conditions, identify the major product.
      </p>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={q.reactionId + q.scenario}
