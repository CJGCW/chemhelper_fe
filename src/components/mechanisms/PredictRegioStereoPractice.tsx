...
type SubMode = 'regio' | 'stereo'

function PracticeCard({ type, category }: { type: 'predict-regio' | 'predict-stereo'; category: MechanismCategory | 'all' }) {
  const makeQ = () => generateQuestion(type, category)
  const [q, setQ]             = useState<MechQuestion | null>(() => makeQ())
  ...
  if (!q) {
    return (
      <p className="font-sans text-sm text-dim py-8 text-center">
        No reactions with {type === 'predict-regio' ? 'regiochemistry' : 'stereochemistry'} data in this category.
      </p>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4 max-w-2xl">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={q.reactionId + type}
          initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
          transition={{ duration: 0.18 }}
          className={`rounded-sm border p-5 flex flex-col gap-4 transition-colors ${
            !checked ? 'border-border' : correct ? 'border-emerald-500 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-950/20' : 'border-rose-500 dark:border-rose-800/50 bg-rose-50 dark:bg-rose-950/20'
          }`}
  ...
  return (
    <div className="flex flex-col gap-5">
      <p className="font-sans text-sm text-secondary leading-relaxed">
        Predict the regiochemistry or stereochemistry of the reaction.
      </p>

      <div className="flex gap-1 print:hidden">
        {(['regio', 'stereo'] as SubMode[]).map(m => {
          const isActive = subMode === m
          const label = m === 'regio' ? 'Regiochemistry' : 'Stereochemistry'
          return (
            <button key={m} onClick={() => setSubMode(m)}
              className="relative px-4 py-1.5 rounded-sm font-sans text-sm transition-colors"
              style={{ color: isActive ? 'var(--c-halogen)' : 'rgba(var(--overlay),0.45)', background: isActive ? activeTint : undefined, border: isActive ? `1px solid ${activeBorder}` : '1px solid transparent' }}
            >
              {label}
            </button>
          )
      ...
      <PracticeCard
        type={subMode === 'regio' ? 'predict-regio' : 'predict-stereo'}
        category={category}
      />
    </div>
  )
