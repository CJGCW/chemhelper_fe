...
import {
  generateDatingProblem, checkDatingAnswer, type DatingProblem,
...
export default function DatingPractice({ allowCustom = true }: Props) {
  const [problem, setProblem] = useState<DatingProblem>(generateDatingProblem)
  ...
  function nextProblem() {
    setProblem(generateDatingProblem())
    setAnswer(''); setCheckState('idle'); setSteps([])
  }

  function handleCheck() {
    if (!answer.trim() || checkState !== 'idle') return
    const correct = checkDatingAnswer(answer, problem)
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <p className="font-sans text-sm text-secondary leading-relaxed">
        Apply carbon-14 dating: <span className="font-mono text-primary">t = (t½ / ln 2) × ln(A₀ / A)</span>.
        Answers accepted within ±1%. Give your answer in years.
      </p>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            />
            <span className="font-mono text-sm text-secondary">years</span>
              ...
              <div className="flex flex-col gap-0.5">
                ...
                {checkState === 'wrong' && (
                  <span className="font-mono text-xs text-dim">
                    Expected ≈ {Math.round(problem.answer).toLocaleString()} years
                  </span>
                )}
              </div>
            )}
