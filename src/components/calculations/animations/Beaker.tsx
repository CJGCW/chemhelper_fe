...
const BEAKER_SIZES = [0.025, 0.05, 0.1, 0.15, 0.25, 0.4, 0.5, 0.6, 1.0, 2.0, 3.0]
function pickBeakerSize(v: number): number {
  if (v <= 0) return 0.25
  return BEAKER_SIZES.find(s => s >= v / 0.85) ?? BEAKER_SIZES[BEAKER_SIZES.length - 1]
}
function fmtVol(l: number): string {
  return l >= 1 ? `${l}L` : `${Math.round(l * 1000)}mL`
}
interface Particle { id: number; x: number; delay: number; size: number }
function genParticles(n: number): Particle[] {
  return Array.from({ length: n }, (_, i) => ({
    id: i, x: 15 + Math.random() * 70, delay: i * 0.07, size: 2 + Math.random() * 2,
  }))
}
...
export default function Beaker({ liquidAmount, concentration, concMax, concUnit, concDisplay, moles, playing, onComplete }: Props) {
  const [particles, setParticles] = useState<Particle[]>([])
  const [stage, setStage] = useState<'idle'|'filled'|'dissolving'|'done'>('idle')
  ...
  const fillFrac    = liquidAmount != null ? Math.min(liquidAmount / beakerSize, 1) : 0
  const concFrac    = concentration != null ? Math.min(concentration / concMax, 1) : 0
  const pCount      = moles != null ? Math.min(Math.max(Math.round(moles*8)+4,5),24) : 0
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(t); if (stirRef.current) clearInterval(stirRef.current) }
  }, [playing])
  ...
  return (
    <div className="flex flex-col items-center gap-2 select-none w-full">
      <svg viewBox="0 0 220 260" className="w-full"
        ...
        {/* ── PARTICLES ── */}
        <AnimatePresence>
          {stage === 'dissolving' && particles.map(p => {
        ...
        {/* ── CONCENTRATION LABEL ── */}
        {concentration != null && stage !== 'idle' && fillFrac > 0 && liqH > 24 && (
          <motion.text
