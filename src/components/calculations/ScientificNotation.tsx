...
function fmtStandard(parts: SciParts, sf: number): string | null {
  ...
  if (Math.abs(exponent) > 14) return null   // too large/small to display meaningfully
  ...
  return (
    <div className="flex items-end gap-2 flex-wrap">
      ...
      <div className="flex flex-col items-start leading-none pb-0.5">
        ...
        <span className="font-mono text-2xl text-dim">× 10</span>
      </div>
    </div>
  )
}
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        <div className="flex flex-col gap-4 p-4 rounded-sm border border-border" style={{ background: '#0e1016' }}>
          <p className="font-mono text-[10px] tracking-[0.15em] text-dim uppercase">Notation Converter</p>
          ...
          {parts ? (
            <div className="flex flex-col gap-3 pt-1">
              <div>
                <p className="font-mono text-[9px] text-dim uppercase tracking-widest mb-2">Scientific Notation</p>
                <SciDisplay parts={parts} sf={sf} />
              </div>

              {stdForm && (
                <div className="border-t border-border pt-3 flex flex-col gap-1">
                  <p className="font-mono text-[9px] text-dim uppercase tracking-widest">Standard Form</p>
                  <span className="font-mono text-xl font-semibold text-primary">{parts.negative && stdForm[0] !== '-' ? '−' : ''}{stdForm}</span>
                </div>
              )}
              {!stdForm && (
                <div className="border-t border-border pt-3">
                  <p className="font-mono text-xs text-dim">Number too large/small to display in standard form.</p>
                </div>
              )}
            ...
            <p className="font-sans text-xs text-dim">Enter a number to convert.</p>
          )}
        </div>

        {/* Quick reference card */}
        <div className="flex flex-col gap-3 p-4 rounded-sm border border-border" style={{ background: '#0e1016' }}>
          <p className="font-mono text-[10px] tracking-[0.15em] text-dim uppercase">Reading Scientific Notation</p>
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-1">
              <p className="font-mono text-xs text-dim">General form</p>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              {[
                ...
                <div key={i} className="flex flex-col gap-0.5 p-2 rounded-sm" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <span className="font-mono text-[11px] text-dim">{item.ex}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Converting — rules */}
      <div className="flex flex-col gap-3">
        <p className="font-mono text-[10px] tracking-[0.15em] text-dim uppercase">Converting</p>
          ...
          {RULES.map((r, i) => (
            <div key={i} className="flex flex-col gap-2 p-3 rounded-sm border border-border" style={{ background: '#0e1016' }}>
              ...
              <div className="flex flex-col gap-0.5 mt-auto pt-2 border-t border-border">
                {r.from && <span className="font-mono text-xs text-dim">{r.from}</span>}
      ...
      {/* Operations */}
      <div className="flex flex-col gap-3">
        <p className="font-mono text-[10px] tracking-[0.15em] text-dim uppercase">Operations</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {OPERATIONS.map((op, i) => (
            <div key={i} className="flex flex-col gap-2 p-3 rounded-sm border border-border" style={{ background: '#0e1016' }}>
              ...
              <p className="font-mono text-xs text-dim">{op.formula}</p>
              <div className="mt-auto pt-2 border-t border-border">
                {op.example.split('\n').map((line, j) => (
                  <p key={j} className="font-mono text-xs" style={{ color: j === 0 ? 'rgba(255,255,255,0.6)' : j === op.example.split('\n').length - 1 ? 'var(--c-halogen)' : 'rgba(255,255,255,0.4)' }}>
                    {line}
                  </p>
                ))}
              </div>
