...
function NavGroup({ label }: { label: string }) {
  return (
    <div className="px-4 pt-5 pb-1.5">
      <span className="font-mono text-[9px] tracking-[0.15em] text-dim uppercase">{label}</span>
    </div>
  )
}
...
function CalcSubItem({ item, onNavigate }: {
  ...
  const isActive = location.pathname === '/calculations' &&
    currentTab === item.tab &&
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-5 border-b border-border shrink-0">
        <div className="flex items-baseline gap-2">
          <span className="font-mono text-lg font-semibold" style={{ color: 'var(--c-halogen)' }}>⚛</span>
          <span className="font-sans font-semibold text-bright tracking-tight">
            Chem<span style={{ color: 'var(--c-halogen)' }}>Helper</span>
          </span>
        </div>
        <p className="font-mono text-[9px] text-dim mt-0.5 tracking-wider">CHEMISTRY TOOLKIT</p>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        ...
        <div>
          ...
          >
            <span className="font-mono text-base leading-none shrink-0 w-4 text-center"
              style={{ color: isCalcActive ? 'var(--c-halogen)' : undefined }}>⚗</span>
            <span className="flex-1">Calculations</span>
            <motion.span
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && (
          <>
            <motion.div key="nav-backdrop" className="fixed inset-0 bg-black/60 z-50 md:hidden"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={onClose} />
            <motion.aside key="nav-drawer"
