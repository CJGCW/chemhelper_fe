...
import { generateArrheniusProblem, checkArrheniusAnswer, type ArrheniusProblem } from '../../utils/kineticsArrheniusPractice'
...
export default function ArrheniusPractice({ allowCustom = true }: Props) {
  const showAnswers = useShowAnswers()
  const [problem, setProblem]     = useState<ArrheniusProblem>(generateArrheniusProblem)
  ...
  function nextProblem() {
    setProblem(generateArrheniusProblem())
    setAnswer(''); setCheckState('idle'); setSteps([])
  }

  function handleCheck() {
    if (!answer.trim() || checkState !== 'idle') return
    const correct = checkArrheniusAnswer(answer, problem)
  ...
  const promptLabel = problem.mode === 'find-Ea' ? 'Ea (kJ/mol)' : 'k₂'

  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      <p className="font-sans text-sm text-secondary leading-relaxed">
        Apply the Arrhenius equation: <span className="font-mono text-primary">ln(k₂/k₁) = −(Ea/R)(1/T₂ − 1/T₁)</span>
      </p>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={problem.reaction.id + problem.mode + problem.T1}
        ...
        >
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <input
              ...
              placeholder={problem.mode === 'find-Ea' ? 'kJ/mol' : 'numeric k'}
      ...
      <p className="font-mono text-xs text-dim">Answers accepted within ±3%.</p>
    </div>
  )
}
