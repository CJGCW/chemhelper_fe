...
export default function EnthalpyPractice({ allowCustom = true }: Props) {
  ...
  function handleCheck() {
    ...
    const c = checkEnthalpyAnswer(problem, input)
    setCorrect(c)
    setChecked(true)
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={problem.equation}
        ...
        >
          ...
          <p className="font-sans text-base text-primary leading-relaxed">
            Calculate ΔHrxn for the following reaction using standard enthalpies of formation:
          </p>
            ...
            <div className="rounded-sm border border-border bg-raised overflow-hidden">
              ...
              {[...problem.reactants, ...problem.products].map((s, i) => (
                <div key={i} className="grid grid-cols-[1fr_auto] gap-x-6 px-3 py-1.5 border-b border-border last:border-b-0">
                  ...
                  <span className="font-mono text-sm text-bright text-right">{s.dhf}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {checked && (
            ...
            >
              <span className="font-mono text-sm font-semibold">
                {correct
                  ? '✓ Correct!'
                  : showAnswers ? `✗ Answer: ${problem.answer} kJ` : '✗ Incorrect — try again'}
              </span>
            </motion.div>
          )}
