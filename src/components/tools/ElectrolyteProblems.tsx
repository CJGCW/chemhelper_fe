...
import {
  ...
  ELECTROLYTE_OPTIONS,
  pickElectrolyte,
} from '../../utils/electrolyteProblems'

export default function ElectrolyteProblems() {
  const [{ q, idx }, setQState] = useState(() => pickElectrolyte())
  const [selected, setSelected] = useState<ElectrolyteStrength | null>(null)
  ...
  const next = useCallback(() => {
    setQState(s => pickElectrolyte(s.idx))
    setSelected(null)
  }, [])

  function choose(s: ElectrolyteStrength) {
    if (answered) return
    setSelected(s)
    setAttempts(a => a + 1)
    if (s === q.answer) setScore(sc => sc + 1)
  }
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="px-4 py-4 flex flex-col gap-1">
            ...
            <p className="font-sans text-sm text-secondary">{q.name}</p>
          </div>

          {/* Options */}
          <div className="px-4 pb-4 grid grid-cols-1 gap-2">
            {ELECTROLYTE_OPTIONS.map(opt => {
              const isSelected  = selected === opt
              const isAnswer    = opt === q.answer
              const showCorrect = answered && isAnswer
              const showWrong   = answered && isSelected && !isAnswer
              const color = ELECTROLYTE_COLOR[opt]
            ...
            {answered && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                ...
                <div className="px-4 py-3 flex flex-col gap-2">
                  ...
                  <p className="font-mono text-xs text-dim">{q.equation}</p>
