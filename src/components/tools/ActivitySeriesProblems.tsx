...
} from '../../utils/activitySeriesProblems'

export default function ActivitySeriesProblems() {
  const [state, setState] = useState<ActivityPickResult>(() => pickActivity())
  ...
  const typeColor = ACTIVITY_TYPE_COLOR[q.type]
        ...
        >
          <div className="px-4 py-3 border-b border-border flex items-center gap-2"
            ...
            <span className="font-mono text-[9px] px-1.5 py-0.5 rounded-sm"
              ...
              {ACTIVITY_TYPE_LABEL[q.type]}
            </span>
          </div>

          <div className="px-4 py-4">
            <p className="font-sans text-sm text-bright leading-relaxed">{q.prompt}</p>
          </div>
            ...
            {answered && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                ...
                <div className="px-4 py-3 flex flex-col gap-2">
                  ...
                  {!correct && (
                    <p className="font-sans text-xs" style={{ color: 'rgb(var(--color-success))' }}>{q.answer}</p>
                  )}
