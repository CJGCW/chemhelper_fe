...
import { genTriangleProblem, checkTriangleAnswer } from '../../utils/electrochemPractice'
import StepsPanel from '../shared/StepsPanel'
import { useShowAnswers } from '../../stores/preferencesStore'

type ProblemType = 'random' | 'ecell-to-dg' | 'ecell-to-k' | 'dg-to-ecell' | 'k-to-ecell'

const TYPES: { id: ProblemType; label: string; formula: string }[] = [
  { id: 'random',       label: 'Random',      formula: '?'     },
  { id: 'ecell-to-dg',  label: 'E° → ΔG°',   formula: '-nFE°' },
  { id: 'ecell-to-k',   label: 'E° → K',      formula: 'lnK'  },
  { id: 'dg-to-ecell',  label: 'ΔG° → E°',   formula: '-ΔG/nF'},
  { id: 'k-to-ecell',   label: 'K → E°',      formula: 'lnK→E'},
]

interface Props { allowCustom?: boolean }

export default function TrianglePractice({ allowCustom = true }: Props) {
  const showAnswers = useShowAnswers()
  const [selected, setSelected]   = useState<ProblemType>('random')
  const [problem, setProblem]     = useState(() => genTriangleProblem('random'))
  ...
  function nextProblem(sel: ProblemType = selected) {
    setProblem(genTriangleProblem(sel))
    setAnswer(''); setChecked(false); setSteps([])
  }

  function handleTypeChange(sel: ProblemType) {
    setSelected(sel)
    setProblem(genTriangleProblem(sel))
    setAnswer(''); setChecked(false); setSteps([])
  ...
  function handleCheck() {
    ...
    const c = checkTriangleAnswer(answer, problem)
    setCorrect(c)
    setChecked(true)
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl">
          ...
          <div className="flex items-stretch gap-0">
            <input
              ...
              className={`bg-raised border rounded-l-sm px-3 py-1.5 font-mono text-base
                          placeholder-dim focus:outline-none transition-colors w-40
                          disabled:cursor-not-allowed
                          ${checked
                            ? correct ? 'border-emerald-500 dark:border-emerald-700/60 text-emerald-700 dark:text-emerald-300' : 'border-rose-500 dark:border-rose-700/60 text-rose-700 dark:text-rose-300'
                            : 'border-border text-bright'}`}
            />
            {problem.unit && (
              <span className={`flex items-center px-2 font-mono text-xs rounded-r-sm border border-l-0 shrink-0
                                ${checked
                                  ? correct ? 'border-emerald-500 dark:border-emerald-700/60 text-emerald-700 dark:text-emerald-400' : 'border-rose-500 dark:border-rose-700/60 text-rose-700 dark:text-rose-400'
      ...
      <p className="font-mono text-xs text-secondary">
        ΔG° = −nFE° · ln K = nFE°/RT · answers accepted within ±1% or ±0.5 log units for K
      </p>
    </div>
  )
}
