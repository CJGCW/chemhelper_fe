...
import { genFaradayProblem, checkFaradayAnswer } from '../../utils/electrochemPractice'
import StepsPanel from '../shared/StepsPanel'
import { useShowAnswers } from '../../stores/preferencesStore'

type ProblemType = 'random' | 'mass' | 'current' | 'time'

const TYPES: { id: ProblemType; label: string; formula: string }[] = [
  { id: 'random',  label: 'Random',          formula: '?'        },
  { id: 'mass',    label: 'Find Mass',        formula: 'm=ItM/nF' },
  { id: 'current', label: 'Find Current',     formula: 'I=mnF/tM' },
  { id: 'time',    label: 'Find Time',        formula: 't=mnF/IM' },
]

interface Props { allowCustom?: boolean }

export default function ElectrolysisPractice({ allowCustom = true }: Props) {
  const showAnswers = useShowAnswers()
  const [selected, setSelected]   = useState<ProblemType>('random')
  const [problem, setProblem]     = useState(() => genFaradayProblem('random'))
  ...
  function nextProblem(sel: ProblemType = selected) {
    setProblem(genFaradayProblem(sel))
  ...
  function handleCheck() {
    ...
    const c = checkFaradayAnswer(answer, problem)
    setCorrect(c)
    setChecked(true)
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl">
      ...
      <p className="font-mono text-xs text-secondary">
        m = (I × t × M) / (n × F) · F = 96,485 C/mol · answers accepted within ±2%
      </p>
    </div>
  )
}
