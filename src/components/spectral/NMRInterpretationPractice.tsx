...
import { NMR_PROBLEMS, type NMRProblem } from '../../data/spectral/nmrProblems'

interface Props {
  mode?: 'practice' | 'problems'
}

function difficultyStyle(d: NMRProblem['difficulty']): React.CSSProperties {
...
export default function NMRInterpretationPractice({ mode = 'practice' }: Props) {
  const pool = useMemo(
    () => NMR_PROBLEMS.filter(p => mode === 'practice' ? p.difficulty !== 'hard' : p.difficulty === 'hard'),
    [mode],
  )

  const [problem, setProblem] = useState<NMRProblem>(() => pool[Math.floor(Math.random() * pool.length)])
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-2 text-[10px] font-mono text-dim flex-wrap">
          {problem.peaks.map(p => (
