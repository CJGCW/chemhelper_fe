...
import { generateWeakAcidProblem, generateDynamicWeakAcidProblem } from '../../utils/acidBasePractice'
...
function nextWeakAcid(allowCustom: boolean) {
  return (!allowCustom || Math.random() < 0.6) ? generateDynamicWeakAcidProblem() : generateWeakAcidProblem()
}

export default function WeakAcidPractice({ allowCustom: _allowCustom = true }: Props) {
  const showAnswers = useShowAnswers()
  const [problem, setProblem] = useState(() => nextWeakAcid(_allowCustom))
  ...
  const next = useCallback(() => {
    setProblem(nextWeakAcid(_allowCustom))
    setAnswer('')
    setVerifyState(null)
    setRevealed(false)
  }, [_allowCustom])
  ...
  return (
    <div className="flex flex-col gap-5 max-w-lg">
      ...
      {(revealed || verifyState) && (
        <PhScale pH={problem.correctPh} label="Answer" />
      )}
