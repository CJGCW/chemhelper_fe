        ...
        <div className="flex flex-col gap-4">
          <div className="p-3 rounded-sm border border-border bg-surface flex flex-col gap-2">
            ...
            <ul className="font-sans text-sm text-secondary flex flex-col gap-1 list-disc list-inside">
              <li>All <span className="text-emerald-400">activators</span> are ortho/para directors</li>
              <li>All <span className="text-rose-400">deactivators</span> are meta directors — <strong className="text-primary">except halogens</strong> (deactivating but o/p directing)</li>
                ...
                {DIRECTING_DATA.map(row => (
                  <tr key={row.sub} className="border-b border-border/50 group">
                    ...
                    <td className="py-2 pr-3">
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${row.type === 'EDG' ? 'bg-emerald-950/40 text-emerald-400' : 'bg-rose-950/40 text-rose-400'}`}>
                        {row.type}
                      </span>
                    </td>
            ...
            <motion.div key={probIdx}
              ...
              {selected !== null && (
                <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                  className={`p-3 rounded-sm border text-sm font-sans ${selected === problem.majorPosition ? 'border-emerald-700/50 bg-emerald-950/20 text-emerald-300' : 'border-rose-700/50 bg-rose-950/20 text-rose-300'}`}>
