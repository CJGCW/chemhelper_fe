...
const PROBLEMS: Problem[] = [
  {
    ...
    choices: [
      { label: 'On the lone pair of the oxygen of HO⁻' },
      { label: 'On the C=O pi bond' },
      { label: 'On the C–H bond of the aldehyde' },
  ...
  },
  {
    ...
    choices: [
      { label: 'Full-headed (two-electron) arrow' },
      { label: 'Half-headed (fishhook) arrow' },
      { label: 'Double-headed resonance arrow' },
      { label: 'Equilibrium arrow' },
    ],
    correct: 1,
  ...
  },
  {
    ...
    choices: [
      { label: 'The leaving group' },
      { label: 'The carbon bonded to the leaving group' },
      { label: 'The lone pair on the leaving group' },
      { label: 'Any adjacent hydrogen' },
    ],
    correct: 1,
  ...
  },
  {
    ...
    choices: [
      { label: 'Atoms move to reflect the resonance contributor' },
      { label: 'Only electrons move — atoms stay fixed' },
      { label: 'Both electrons and the H atom move between the two oxygens' },
      { label: 'Each resonance form is a separate real molecule' },
    ],
    correct: 1,
  ...
  },
  {
    ...
    choices: [
      { label: 'Tail at O–H bond; head at water oxygen' },
      { label: 'Tail at water lone pair; head at H of O–H' },
      { label: 'Tail at C=O pi bond; head at water oxygen' },
      { label: 'Tail at water oxygen; head at the methyl carbon' },
    ],
    correct: 1,
  ...
  },
  {
    ...
    choices: [
      { label: 'At the oxygen lone pair' },
      { label: 'At the C=C pi bond (between the two carbons)' },
      { label: 'At the C–O single bond' },
      { label: 'At the H being removed' },
    ],
    correct: 1,
  ...
  },
  {
    ...
    choices: [
      { label: 'One fishhook arrow from C⁺ to Br' },
      { label: 'One full-headed arrow from a Br⁻ lone pair to C⁺' },
      { label: 'Two arrows: Br lone pair → C, and C–C bond → adjacent C' },
      { label: 'No arrows are needed — ions attract automatically' },
    ],
    correct: 1,
  ...
  },
  {
    ...
    choices: [
      { label: 'It breaks completely — the C–O bond is lost' },
      { label: 'The pi bond electrons move to oxygen, giving O a negative charge' },
      { label: 'Nothing — the carbonyl is unchanged' },
      { label: 'The pi bond becomes a triple bond' },
    ],
    correct: 1,
...
export default function CurvedArrowPractice({ allowCustom = true }: Props) {
  ...
  const correctLabel = problem.choices[problem.correct].label
  const correct = submitted && selected === correctLabel
        ...
        <div className="flex flex-col gap-2">
          {problem.choices.map((choice, ci) => (
            <RenderableChoiceButton
              key={choice.label}
              choice={{ label: `${String.fromCharCode(65 + ci)}. ${choice.label}` }}
              isSelected={selected === choice.label}
              isCorrect={ci === problem.correct}
              isChecked={submitted}
              onSelect={() => setSelected(choice.label)}
            />
          ))}
        </div>
