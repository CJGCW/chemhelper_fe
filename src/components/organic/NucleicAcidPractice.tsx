...
const SUB_MODES: { id: NucleicAcidProblemType; label: string; formula: string }[] = [
  { id: 'complement', label: 'Base Pairing',  formula: 'A↔T' },
  { id: 'base-id',    label: 'Base Identity', formula: 'pur?' },
  { id: 'dna-vs-rna', label: 'DNA vs RNA',    formula: 'U/T?' },
]

function newQuestion(type: NucleicAcidProblemType): NucleicAcidProblem {
  return generateNucleicAcidProblem(type)
}

export default function NucleicAcidPractice({ allowCustom: _allowCustom = true }: Props) {
  const [subMode, setSubMode]     = useState<NucleicAcidProblemType>('complement')
  const [question, setQuestion]   = useState<NucleicAcidProblem>(() => newQuestion('complement'))
  const [selected, setSelected]   = useState<string | null>(null)
  const [checked, setChecked]     = useState(false)
  const [showSteps, setShowSteps] = useState(false)
  const [score, setScore]         = useState({ correct: 0, total: 0 })

  function changeMode(mode: NucleicAcidProblemType) {
    setSubMode(mode)
  ...
  function handleSelect(opt: string) {
    if (checked) return
    setSelected(opt)
    const correct = checkNucleicAcidAnswer(question, opt)
    setChecked(true)
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl">
        ...
        {SUB_MODES.map(m => {
          ...
          return (
            <button key={m.id} onClick={() => changeMode(m.id)}
              ...
              {isActive && (
                <motion.div layoutId="nucleic-sub-pill" className="absolute inset-0 rounded-sm"
      ...
      <AnimatePresence mode="wait">
        <motion.div key={`${subMode}-${question.scenario.slice(0, 20)}`}
          ...
          {checked && (
            <div className="flex flex-col gap-1.5">
              ...
              <button onClick={() => setShowSteps(o => !o)}
                ...
                {showSteps ? '▲ Hide steps' : '▼ Show reasoning'}
              </button>
            </div>
          )}
        </motion.div>
        ...
        {showSteps && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
            ...
            <div className="rounded-sm border border-border p-4 flex flex-col gap-2" style={{ background: 'rgb(var(--color-base))' }}>
              <span className="font-mono text-[10px] text-dim uppercase tracking-wider">Reasoning</span>
